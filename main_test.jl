...
function output_func(sol, i)
    ...
    sqrt_3 = sqrt(3)
    sz_traj = Vector{Float64}(undef, length(sol.t))
    @inbounds for (j, t) in enumerate(sol.t)
        ...
        sz_traj[j] = sqrt_3 * sum_cos / nAtoms
    end
    traj_idx = 0
    lock(file_lock) do
        jldopen(sol_filename, "a+") do file
            # Ensure sz group exists
            if !haskey(file, "sz")
                JLD2.Group(file, "sz")
            end
            # Get highest existing trajectory index
            sz_group = file["sz"]
            existing_traj = isempty(keys(sz_group)) ? 0 : maximum(parse(Int, match(r"traj_(\d+)", k).captures[1]) for k in keys(sz_group) if occursin(r"traj_\d+", k))
            traj_idx = existing_traj + i
            file["sz/traj_$traj_idx"] = sz_traj
            # Write tSave if not present
...
function compute_and_save_sz_mean(src_file, dest_file, nTraj)
    sz_mean = nothing
    t_save = nothing
    lock(file_lock) do
        jldopen(src_file, "r") do file
            ...
            n_traj = length(traj_keys)
            first_traj = sz_group[traj_keys[1]]
            n_times = length(first_traj)
            sz_mean = zeros(Float64, n_times)
    ...
    lock(file_lock) do
        jldopen(dest_file, "w") do file
            file["sz_mean"] = sz_mean
            file["tSave"] = t_save
        end
    end
...
let
    ...
    @assert case in [1, 2, 3] "case must be 1 (1D) or 2 (2D) or 3 (3D)"
    traj_counter[] = 0
    ...
    println("Starting ensemble solve with $nTraj new trajectories (existing: $existing_traj)...")
    flush(stdout)
    @time computeTWA(nAtoms, tf, nT, nTraj, Ω, Δ, V, Γ, γ, case)
    println("Finished computeTWA.")
    flush(stdout)

    # Compute and save mean sz
    ...
    println("Computing mean sz and saving to new file...")
    flush(stdout)
    compute_and_save_sz_mean(src_file, dest_file, existing_traj + nTraj)
end
